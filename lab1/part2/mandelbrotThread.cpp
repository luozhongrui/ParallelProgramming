...
void workerThreadStart(WorkerArgs *const args) {
  ...
  // }
  // endTime = clock();
  // printf("thread %d Time : %5f s\n", args->threadId,
  //        (double)(endTime - startTime) / CLOCKS_PER_SEC);
  //   printf("Hello world from thread %d\n", args->threadId);
  clock_t startTime, endTime;
  startTime = clock();
  int eachhight = args->height / args->numThreads;
  int remaind = args->height % args->numThreads;
  if (args->numThreads - 1 == args->threadId) {
    // eachhight += remaind;
    mandelbrotSerial(args->x0, args->y0, args->x1, args->y1, args->width,
                     args->height, args->threadId * eachhight,
                     eachhight + remaind, args->maxIterations, args->output);
  } else {
    mandelbrotSerial(args->x0, args->y0, args->x1, args->y1, args->width,
                     args->height, args->threadId * eachhight, eachhight,
